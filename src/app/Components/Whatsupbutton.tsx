...
export default function WhatsAppButton() {
  ...
  return (
    <>
      ...
      >
        {/* Pulse Ring */}
        <span className="absolute inset-0 w-full h-full rounded-full bg-green-500 opacity-30 animate-ping"></span>
          ...
          {isHovered && (
            <div
              className={`
                ...
                bg-ultracom-dark text-white text-sm font-medium px-4 py-2 rounded-lg
                whitespace-nowrap opacity-0 group-hover:opacity-100
                transition-opacity duration-300 pointer-events-none
              `}
            >
              Chat with us on WhatsApp
              <span className="absolute left-full top-1/2 -translate-y-1/2 w-0 h-0 border-t-8 border-b-8 border-l-8 border-transparent border-l-ultracom-dark"></span>
            </div>
          )}
        </div>
      </a>
